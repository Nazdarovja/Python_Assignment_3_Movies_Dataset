...
def largest_budget_movie(movies_df):
    ...
    movie = movies_df.iloc[pd.to_numeric(movies_df['budget'], errors='coerse', downcast='integer').idxmax()] ## jesus.. what have i done...
...
def most_popular_dk_movie(movies_df):
    ...
    movie = movies_df.iloc[pd.to_numeric(movies_df[movies_df['production_countries'].str.contains('Denmark', na=False)]['popularity'], errors='coerse').idxmax()]
   
    return (movie.title, movie.popularity)


def biggest_revenue_english_action_movie(movies_df):
    ## guess this is the magic to rule them all :D 
